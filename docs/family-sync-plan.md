...
- the family sync worker now routes each invite code through a Durable Object instead of KV, preserving the `GET`/`PUT`/`DELETE` contract expected by the current HTTP client while removing cross-edge eventual-consistency lag
...
- that backend smoke path should cover authenticated same-user rejoin too, so the worker contract is validated for identity reconciliation even before the browser E2E runs
- the smoke script should also support reproducible debug runs for operators, including a fixed group code and a way to disable the auth-rejoin leg when isolating backend failures
...
- keep that built-in auth bridge synchronized across tabs via storage events, so a sign-in or sign-out flow in one tab can rehydrate family sync identity in another without a full reload
