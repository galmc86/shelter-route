import { beforeEach, describe, expect, it, vi } from 'vitest';
import { FAMILY_REMOTE_GATEWAY_STORAGE_KEY } from '../familyRemoteGatewayModeService';

describe('familyRemoteGatewayModeService', () => {
  beforeEach(() => {
    localStorage.clear();
    vi.resetModules();
    vi.unstubAllEnvs();
  });

  it('defaults to the mock gateway', async () => {
    vi.stubEnv('VITE_FAMILY_REMOTE_URL', '');
    vi.stubEnv('VITE_FAMILY_REMOTE_GATEWAY', '');
    const { getFamilyRemoteGatewayMode } = await import('../familyRemoteGatewayModeService');

    expect(getFamilyRemoteGatewayMode()).toBe('mock');
  });

  it('accepts a backend mode override from the query string', async () => {
    const { initializeFamilyRemoteGatewayModeFromUrl } = await import('../familyRemoteGatewayModeService');

    expect(initializeFamilyRemoteGatewayModeFromUrl('?familyRemoteGateway=backend')).toBe('backend');
    expect(localStorage.getItem(FAMILY_REMOTE_GATEWAY_STORAGE_KEY)).toBe('backend');
  });

  it('falls back to the env gateway mode when no query or storage override is present', async () => {
    vi.stubEnv('VITE_FAMILY_REMOTE_GATEWAY', 'backend');
    const { getFamilyRemoteGatewayMode } = await import('../familyRemoteGatewayModeService');

    expect(getFamilyRemoteGatewayMode()).toBe('backend');
  });

  it('defaults to the backend gateway when a family remote URL is configured', async () => {
    vi.stubEnv('VITE_FAMILY_REMOTE_URL', 'https://family-sync.example');
    const { getFamilyRemoteGatewayMode } = await import('../familyRemoteGatewayModeService');

    expect(getFamilyRemoteGatewayMode()).toBe('backend');
  });
});
