...
describe('computeRoutes', () => {
  ...
  it('sorts routes by duration and marks fastest', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockResponse({
        routes: [
          { geometry: SIMPLE_POLYLINE, summary: { duration: 900, distance: 1200 } },
          { geometry: SIMPLE_POLYLINE, summary: { duration: 600, distance: 800 } },
          { geometry: SIMPLE_POLYLINE, summary: { duration: 750, distance: 1000 } },
        ],
      })
    );
    ...
    expect(result[0].durationSeconds).toBeLessThanOrEqual(result[1].durationSeconds);
    expect(result[1].durationSeconds).toBeLessThanOrEqual(result[2].durationSeconds);
    // Fastest is marked
